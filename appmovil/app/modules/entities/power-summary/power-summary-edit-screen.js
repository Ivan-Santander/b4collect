...
function PowerSummaryEditScreen(props) {
  const { getPowerSummary, updatePowerSummary, route, powerSummary, fetching, updating, errorUpdating, updateSuccess, navigation, reset } =
    props;
  ...
  React.useEffect(() => {
    if (!isNewEntity) {
      getPowerSummary(route.params.entityId);
    } else {
      reset();
    }
  }, [isNewEntity, getPowerSummary, route, reset]);

  React.useEffect(() => {
    if (isNewEntity) {
      setFormValue(entityToFormValue({}));
    } else if (!fetching) {
      setFormValue(entityToFormValue(powerSummary));
    }
  }, [powerSummary, fetching, isNewEntity]);
    ...
    if (updating === false) {
      if (errorUpdating) {
        ...
        isNewEntity || !navigation.canGoBack()
          ? navigation.replace('PowerSummaryDetail', { entityId: powerSummary?.id })
          : navigation.pop();
      }
    }
  }, [updateSuccess, errorUpdating, navigation]);

  const onSubmit = (data) => updatePowerSummary(formValueToEntity(data));
  ...
  return (
    <View style={styles.container}>
      <KeyboardAwareScrollView
        ...
        testID="powerSummaryEditScrollView"
...
const mapStateToProps = (state) => {
  return {
    ...
    updating: state.powerSummaries.updating,
    updateSuccess: state.powerSummaries.updateSuccess,
    errorUpdating: state.powerSummaries.errorUpdating,
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    updatePowerSummary: (powerSummary) => dispatch(PowerSummaryActions.powerSummaryUpdateRequest(powerSummary)),
    reset: () => dispatch(PowerSummaryActions.powerSummaryReset()),
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(PowerSummaryEditScreen);
