...
function SpeedSummaryEditScreen(props) {
  const { getSpeedSummary, updateSpeedSummary, route, speedSummary, fetching, updating, errorUpdating, updateSuccess, navigation, reset } =
    props;
  ...
  React.useEffect(() => {
    if (!isNewEntity) {
      getSpeedSummary(route.params.entityId);
    } else {
      reset();
    }
  }, [isNewEntity, getSpeedSummary, route, reset]);

  React.useEffect(() => {
    if (isNewEntity) {
      setFormValue(entityToFormValue({}));
    } else if (!fetching) {
      setFormValue(entityToFormValue(speedSummary));
    }
  }, [speedSummary, fetching, isNewEntity]);
    ...
    if (updating === false) {
      if (errorUpdating) {
        ...
        isNewEntity || !navigation.canGoBack()
          ? navigation.replace('SpeedSummaryDetail', { entityId: speedSummary?.id })
          : navigation.pop();
      }
    }
  }, [updateSuccess, errorUpdating, navigation]);

  const onSubmit = (data) => updateSpeedSummary(formValueToEntity(data));
  ...
  return (
    <View style={styles.container}>
      <KeyboardAwareScrollView
        ...
        testID="speedSummaryEditScrollView"
...
const mapStateToProps = (state) => {
  return {
    ...
    updating: state.speedSummaries.updating,
    updateSuccess: state.speedSummaries.updateSuccess,
    errorUpdating: state.speedSummaries.errorUpdating,
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    updateSpeedSummary: (speedSummary) => dispatch(SpeedSummaryActions.speedSummaryUpdateRequest(speedSummary)),
    reset: () => dispatch(SpeedSummaryActions.speedSummaryReset()),
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(SpeedSummaryEditScreen);
