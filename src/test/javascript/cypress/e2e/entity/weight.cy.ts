...
describe('Weight e2e test', () => {
  ...
  describe('new Weight page', () => {
    ...
    it('should create an instance of Weight', () => {
      cy.get(`[data-cy="usuarioId"]`).type('Central Andalucía calculating').should('have.value', 'Central Andalucía calculating');

      cy.get(`[data-cy="empresaId"]`).type('Marketing Guantes').should('have.value', 'Marketing Guantes');

      cy.get(`[data-cy="fieldWeight"]`).type('86687').should('have.value', '86687');

      cy.get(`[data-cy="endTime"]`).type('2023-03-23T21:54').blur().should('have.value', '2023-03-23T21:54');
