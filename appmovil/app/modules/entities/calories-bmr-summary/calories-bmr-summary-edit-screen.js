...
function CaloriesBmrSummaryEditScreen(props) {
  const {
  ...
  React.useEffect(() => {
    if (!isNewEntity) {
      getCaloriesBmrSummary(route.params.entityId);
    } else {
      reset();
    }
  }, [isNewEntity, getCaloriesBmrSummary, route, reset]);

  React.useEffect(() => {
    if (isNewEntity) {
      setFormValue(entityToFormValue({}));
    } else if (!fetching) {
      setFormValue(entityToFormValue(caloriesBmrSummary));
    }
  }, [caloriesBmrSummary, fetching, isNewEntity]);
    ...
    if (updating === false) {
      if (errorUpdating) {
        ...
        isNewEntity || !navigation.canGoBack()
          ? navigation.replace('CaloriesBmrSummaryDetail', { entityId: caloriesBmrSummary?.id })
          : navigation.pop();
      }
    }
  }, [updateSuccess, errorUpdating, navigation]);

  const onSubmit = (data) => updateCaloriesBmrSummary(formValueToEntity(data));
  ...
  return (
    <View style={styles.container}>
      <KeyboardAwareScrollView
        ...
        testID="caloriesBmrSummaryEditScrollView"
...
const mapStateToProps = (state) => {
  return {
    ...
    updating: state.caloriesBmrSummaries.updating,
    updateSuccess: state.caloriesBmrSummaries.updateSuccess,
    errorUpdating: state.caloriesBmrSummaries.errorUpdating,
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    updateCaloriesBmrSummary: (caloriesBmrSummary) =>
      dispatch(CaloriesBmrSummaryActions.caloriesBmrSummaryUpdateRequest(caloriesBmrSummary)),
    reset: () => dispatch(CaloriesBmrSummaryActions.caloriesBmrSummaryReset()),
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(CaloriesBmrSummaryEditScreen);
