...
function ActiveMinutesEditScreen(props) {
  const {
    getActiveMinutes,
    updateActiveMinutes,
    route,
  ...
  React.useEffect(() => {
    if (!isNewEntity) {
      getActiveMinutes(route.params.entityId);
    } else {
      reset();
    }
  }, [isNewEntity, getActiveMinutes, route, reset]);

  React.useEffect(() => {
    if (isNewEntity) {
      setFormValue(entityToFormValue({}));
    } else if (!fetching) {
      setFormValue(entityToFormValue(activeMinutes));
    }
  }, [activeMinutes, fetching, isNewEntity]);
    ...
    if (updating === false) {
      if (errorUpdating) {
        ...
        isNewEntity || !navigation.canGoBack()
          ? navigation.replace('ActiveMinutesDetail', { entityId: activeMinutes?.id })
          : navigation.pop();
      }
    }
  }, [updateSuccess, errorUpdating, navigation]);

  const onSubmit = (data) => updateActiveMinutes(formValueToEntity(data));
  ...
  return (
    <View style={styles.container}>
      <KeyboardAwareScrollView
        ...
        testID="activeMinutesEditScrollView"
...
const mapStateToProps = (state) => {
  return {
    ...
    updating: state.activeMinutes.updating,
    updateSuccess: state.activeMinutes.updateSuccess,
    errorUpdating: state.activeMinutes.errorUpdating,
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    updateActiveMinutes: (activeMinutes) => dispatch(ActiveMinutesActions.activeMinutesUpdateRequest(activeMinutes)),
    reset: () => dispatch(ActiveMinutesActions.activeMinutesReset()),
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(ActiveMinutesEditScreen);
